    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <h1>CERCA UTENTE</h1>
                <br>
                <form class="form-inline" action = "cercaUtente.php" method = "post">
                    <div class="input-group">
                        <input type="search" class="form-control" placeholder="Inserisci username" name="UserCercato" id="cercaUtente">
                        <div class="input-group-btn">
                            <input type="submit" class="btn btn-default" value="Cerca" name="submit">
                        </div>
                    </div>
                </form>
                <br>
                <?php
                    if (isset($_POST['submit'])) {
                        if (empty(trim($_POST['UserCercato']))){                              
                            echo '<p id="errore">Non hai cercato nessun Username</p>';
                            exit();
                        } else {
                            $UserCercato = trim($_POST["UserCercato"]);
                            $trovato = 0;
                            ...
                            while ($row = $utente -> fetch()){
                                if (trim($UserCercato) == $row["Username"]){
                                    $trovato = 1;
                                }
                            }

                            if ($trovato == 1){
                                $_SESSION["UserCercato"] = $UserCercato;
                                header('Location:profiloGenerico.php');
                            } else {
                                echo '<p id="errore">Username non trovato.</p>';
                                exit();
                            }
                        }
                    }
